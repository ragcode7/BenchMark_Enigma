...
import com.benchmarkChat.chatapp.MainMenu.MenuActivity;
                ...
                .addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()){
                            ...
                            System.out.println("====dataaa"+hashMap);

                            User model = new User();
                            model.setId(userid);
                            model.setUsername(username);
                            model.setImageURL("default");
                            model.setStatus("offline");
                            model.setBio("");
                            model.setSearch(username.toLowerCase());
                            System.out.println("====dataaa"+hashMap);
                            if(dialog!=null){
                                dialog.dismiss();
                            }
                            reference.child(FirebaseAuth.getInstance().getUid()).setValue(model).addOnCompleteListener(new OnCompleteListener<Void>() {
                                ...
                                public void onComplete(@NonNull Task<Void> task) {
                                    if (task.isSuccessful()){
                                        ...
                                        System.out.println("====dataaa==failure");
                                    }
                                }
                            });
